...
const payload = Object.freeze({
  ...
  messageVersion: "v1",
  sequenceStep: "FU1",
  sequenceInstanceKey: "SEQ|campaign|lead|v1",
  sequenceVersionSnapshot: "seq-v1",
  templateVersionSnapshot: "tpl-v3",
  senderIdentitySnapshot: "sender:jef-outreach",
  verifiedRecipient: "person@example.com",
  destination: "person@example.com",
  finalSubjectSnapshot: "Hello",
  subject: "Hello",
  finalBodySnapshot: "Body",
  textBody: "Body",
...
const canonicalBinding = Object.freeze({
  decision: "AUTHORIZED",
  commandId: "CMD-EXACT-EFFECT",
  releaseId: "REL-OUTREACH-V2",
  authorityVersion: "v1",
  effectKey,
  payloadFingerprint: canonicalPayloadFingerprint(payload),
  verifiedRecipient: payload.verifiedRecipient,
  senderIdentity: payload.senderIdentitySnapshot,
  correlationDomain: "outreach.example.test"
});

function harness({
  claimResult = "WON",
  safety = true,
  providerResult = { confirmed: true, providerMessageId: "gmail-1" },
  providerError,
  ...
  const adapter = new GmailOutreachV2Adapter({
    ...
    attemptStore: {
      async reserve({ identity, payloadFingerprint }) {
        effects.reserve++;
      ...
      async unknownHold(args) { effects.unknown++; record = { ...record, ...args, state: "UNKNOWN_HOLD" }; },
      async failNoRetry(args) { effects.fail++; record = { ...record, ...args, state: "FAILED_NO_RETRY" }; },
...
test("confirmed send binds canonical authority/payload, reserves once, and persists Gmail outcome", async () => {
  const h = harness();
...
test("canonical authority cannot be manufactured by a bare boolean or mismatched effect", async () => {
  for (const binding of [
    { ...canonicalBinding, decision: "HOLD" },
    { ...canonicalBinding, commandId: "" },
    { ...canonicalBinding, effectKey: "other" }
  ]) {
...
test("recipient/sender/content snapshot tampering blocks before claim/provider", async () => {
  for (const mutated of [
    { ...payload, destination: "wrong@example.com" },
    { ...payload, subject: "Changed" },
    { ...payload, textBody: "Changed" },
    { ...payload, senderIdentitySnapshot: "sender:other" },
    { ...payload, priorContactSnapshot: "UNKNOWN" }
  ]) {
    const h = harness();
...
test("ambiguous acknowledgment becomes UNKNOWN_HOLD and retry cannot reserve or resend", async () => {
  ...
  assert.equal(result.result, "UNKNOWN_HOLD");
  assert.equal(result.automaticRetry, false);
  assert.equal(result.classification, "AMBIGUOUS_PROVIDER_ACK");
  await assert.rejects(h.adapter.execute(request), FailClosedError);
...
test("pre-invocation provider failure is explicit and never converted to UNKNOWN_HOLD", async () => {
  ...
  assert.equal(h.effects.fail, 1);
  assert.equal(h.effects.unknown, 0);
  assert.equal(h.record().state, "FAILED_NO_RETRY");
});

test("unclassified post-boundary provider error fails closed as UNKNOWN_HOLD with explicit classification", async () => {
  const h = harness({ providerError: new Error("unexpected") });
  const result = await h.adapter.execute(request);
  assert.equal(result.result, "UNKNOWN_HOLD");
  assert.equal(result.classification, "UNCLASSIFIED_PROVIDER_ERROR_AFTER_BOUNDARY");
  assert.equal(result.automaticRetry, false);
  assert.equal(h.record().state, "UNKNOWN_HOLD");
});
