...
runtime = Path("netlify/functions/slice01.mts")
s = runtime.read_text()
s = replace_once(
    s,
...
s = replace_once(
    ...
    "canonical sendRaw transport",
)

marker = "function canonicalGmailTransport() {"
helper = r'''function gmailHeader(message: any, name: string) {
...
'''
if marker not in s:
    raise AssertionError("canonicalGmailTransport marker missing")
...
adapter = Path("outreach-provider-adapter/src/adapter.js")
a = adapter.read_text()
a = replace_once(
    a,
...
runtime_test = Path("test/outreach-runtime-guard.test.mjs")
t = runtime_test.read_text()
if "runtime binds Follow-Up Gmail thread and fresh mailbox reality before adapter execution" not in t:
